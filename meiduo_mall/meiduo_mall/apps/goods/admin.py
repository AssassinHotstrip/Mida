...
class GoodsCategoryAdmin(admin.ModelAdmin):
    """自定义站点管理类,用于监听保存和删除事件"""
    def save_model(self, request, obj, form, change):
        """
        当GoodsCategory模型的数据在admin中做了保存操作,则自动调用此方法
        Given a model instance save it to the database.
        :param request: 本次保存时的请求对象
        :param obj: 本次要保存的对象
        :param form: 本次要修改的表单信息
        :param change: 两个数据之间的变化
        :return: None
        """
        obj.save()
        # 保存完之后立即重新生成list.html静态文件(监听到保存事件即执行异步任务)
    ...
    def delete_model(self, request, obj):
        ...
        当GoodsCategory模型的数据在admin中做了删除操作,则自动调用此方法
        :param request: 本次删除时的请求对象
        :param obj: 本次要删除的对象
        :return: None
        """
        obj.delete()
        # 删除完之后立即重新生成list.html静态文件
        generate_static_list_search_html.delay()
