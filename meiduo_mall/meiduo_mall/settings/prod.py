...
# print(BASE_DIR) : /home/python/PythonLearn/Django/meiduo_hzt/meiduo_mall/meiduo_mall
# 给系统导包列表中多加一个导包路径
...
SECRET_KEY = '-1unfca68%*q%zr7n#9zvlq#9^46$-d2e@0mc^d$)d86t7gc#f'
...
# 指定默认的用户模型类
# String model references must be of the form 'app_label.ModelName' 指定用户认证模型必须以  应用名.模型名  写法
...
CRONJOBS = [
    # */分*/时*/日*/月*/周
    # 每1分钟执行一次生成主页静态文件, 执行文件中的该函数, 执行日志保存位置
    ('*/1 * * * *', 'contents.crons.generate_static_index_html', '>> /home/python/PythonLearn/Django/meiduo_hzt/meiduo_mall/logs/crontab.log')
]
...
HAYSTACK_CONNECTIONS = {
    'default': {
        ...
        'URL': 'http://192.168.170.211:9200/',  # 此处为elasticsearch运行的服务器ip地址，端口号固定为9200
        'INDEX_NAME': 'meiduo_20',  # 指定elasticsearch建立的索引库的名称
    },
}
